...
- **Vale refeição** é uma carteira à parte (`method: 'vr'`), não uma linha do caixa. O crédito não entra em `totals.income` e o gasto não entra em `totals.expense`, então a sobra do mês não infla com benefício que só dá para gastar em comida. Os dois aparecem em `vrIn`/`vrOut`, e `vrBalance(ciclo)` acumula desde o primeiro lançamento — o que não foi usado fica para o mês seguinte, como no cartão de verdade. O crédito se cadastra como receita fixa mensal com "Onde cai: Vale refeição".
